...
function VideoDetails() {
  ...
  if (isRecommendedLoading) return <div>Loading...</div>;
  ...
  return (
    <div className="flex flex-row gap-6  w-full px-4 py-6">
      ...
      <div className="flex flex-col basis-[70%] max-w-[900px]">
        ...
        {/* Video Title */}
        <h1 className="text-xl font-bold text-white mb-2">{statsData.title}</h1>

        {/* Video Description */}
        <div className="bg-[#181818] rounded-lg p-4 mb-4 text-gray-200 flex flex-col gap-2">
          <span className="flex gap-2">
            <p>{statsData.number_of_views} Views</p>
        ...
        <div className="bg-[#181818] rounded-lg p-4">
          <h2 className="text-lg font-semibold text-white mb-3">{commentsData.total_number_of_comments} Comments</h2>
          <div className="flex flex-col gap-3">
            {commentsData.comments.map((comment, idx) => (
              <div
                key={idx}
                className="flex gap-2 items-center bg-[#232323] rounded-md p-2"
              >
                                  <img
                    src={comment.thumbnails[0].url}
                    alt={comment.author_name}
                    className="w-8 h-8 rounded-full cursor-pointer"
                    onClick={() => handleChannelClick(comment.author_channel_id)} 
                  />
                <div className="flex flex-col">
                <div className="flex gap-2 items-center">
                <span className="font-bold text-sm text-blue-300" onClick={() => handleChannelClick(comment.author_channel_id)} >
                  {comment.author_name}
                </span>
      ...
      <div className="flex flex-col basis-[27%] gap-4 " >
        <h2 className="text-lg font-semibold text-white mb-2">Recommended</h2>
